    		...
    		<div class="row">
    			<div class="col-xl-10 mx-auto">
                    @if(\App\Models\SystemConfiguration::where('type', 'project_approval')->first()->value == 1)
                        @php $projects = \App\Models\Project::where('paid', 1)->latest()->get()->take(3); @endphp
                    {{-- @else
                        @php $projects = \App\Models\Project::where('paid',1)->latest()->get()->take(3); @endphp --}}
                    @endif
    				@foreach ($projects as $project)
                        <a href="{{ route('project.resources', $project->id)}}" class="d-block card-project card p-4 text-inherit mb-0">
        					<div class="row">
                                <div class="col-8">
                                    ...
                                    <ul class="list-inline opacity-70 fs-12 mb-0">
                                        ...
                                        <li class="list-inline-item">
                                            ...
                                            <span>{{ App\Models\Category::where('id',$project->category_id)->first()->name}}</span>
                                        </li>
                                        <li class="list-inline-item">
                                ...
                                <div class="col-4 text-right">
                                    <span class="h5">{{ $project->freelancer_pay }} Dollars</span>
                                </div>
                            </div>
        				</a>
                    @endforeach
                            ...
                            @if (($category = \App\Models\ProjectCategory::find($category_id)) != null)
                                <div class="col-xl-2 col-lg-3 col-md-4 col-6">
                    				<a class="text-center py-3 px-2 d-block card text-inherit shadow-none bg-light" href="{{ route('projects.category', $category->slug) }}">
                    					<img src="{{ my_asset($category->photo) }}" class="mw-100 h-50px mb-2">
